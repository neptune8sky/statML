...
begin
	data = load_iris() # The famous flower dataset we will use for classification
	iris = DataFrame(data)
...
# ╔═╡ 6461bcc4-d652-499e-92b3-131d042e9d81
function one_hot_encode(data; typ="test")
	idxs = Dict("virginica"=>1, "setosa" => 2, "versicolor" => 3)
	vect = zeros(3)
	if typ == "pred"
		vect[idxs[mode(data)]] = 1
	else
		vect[idxs[data]] = 1
	end
	return vect
...
# ╔═╡ e6fe2bd0-ede2-4a1d-8e90-e049372cb245
function ZeroOneLoss(y, y_hat)
	return sum([sum(row .* row_pred) for (row, row_pred) in (y,y_hat)])
end

# ╔═╡ 90b32741-6d80-4292-8c9f-81ce97103265
begin
	kNNClassifier = @iload KNNClassifier verbosity = false;
	neighbors_range = 1:30
	models = [kNNClassifier(K=num_neighbors) for num_neighbors in neighbors_range]
end
...
begin
    machs = [machine(model, X, y) for model in models] # define machine: binds model to data
    machs = [fit!(mach; rows=train) for mach in machs]# fit model on train set
end

# ╔═╡ 0c377581-b8e9-4f72-8fa3-830ea034c181
begin
	losses = zeros(length(neighbors_range))
...
git-tree-sha1 = "d2c1ab65dbfb02a35442dfc479809a321a44e765"
